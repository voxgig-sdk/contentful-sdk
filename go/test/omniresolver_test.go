...
import (
	...
	omni "github.com/voxgig-sdk/contentful-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ContentfulError); is {
				out := map[string]any{
					"name":    "ContentfulError",
...
func omniCtx(arg any, client *sdk.ContentfulSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ContentfulSDK); is {
			client = live
